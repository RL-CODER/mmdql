...
class ParticleMMD(TD):
    def __init__(self, policy, mdp_info, learning_rate, n_approximators=10, update_mode='deterministic',
                 update_type='weighted', q_min=0, q_max=1, init_values=None, delta =0.1, k_value=1):
        ...
        if init_values is None:
            initial_values = k_value * np.linspace(q_min, q_max, n_approximators) + (1 - k_value) * np.random.uniform(q_min, q_max, n_approximators)
            init_values = np.sort(initial_values)
        for i in range(len(self.Q.model)):
            self.Q.model[i].table = np.tile([init_values[i]], self.Q[i].shape)
        super(ParticleMMD, self).__init__(mdp_info, policy, self.Q, learning_rate)                                 
        self.alpha = [deepcopy(self.alpha)] * n_approximators

    def _update(self, state, action, reward, next_state, absorbing):
        raise NotImplementedError    

class ParticleMMDQLearning(ParticleMMD):
    @staticmethod
        ...
        if absorbing:
            ...
            if self._update_mode == 'deterministic':
                if self._update_type == 'mean':
                    ...
                    prob = ParticleMMDQLearning._compute_prob_max(q_next_all)
            ...
            for i in range(self._n_approximators):
                self.Q.model[i][state, action] = q_current[i] + self.alpha[i](state, action) * (reward + self.mdp_info.gamma * q_next[i] - q_current[i])